...
from internal.config import WAIT_BETWEEN_ACCOUNTS, THREADS_NUM, MAX_TRIES, \
    ...
    DO_GALXE_TASKS, GALXE_CAMPAIGN_IDS, DO_ONCHAIN
...
async def process_account(account_data: Tuple[int, Tuple[str, str, str, str]],
    ...
    if DO_ONCHAIN:
        async with OnchainAccount(idx, account_info, evm_wallet) as onchain:
            try:
                if not account_info.drip_bera:
                    await onchain.drip_bera()
                if not account_info.swap_bera:
                    await onchain.swap_bera()
                if not account_info.mint_honey:
                    await onchain.mint_honey()
            except Exception as onchain_exc:
                await log_long_exc(idx, 'On-chain error', onchain_exc, to_file=False)
                excs.append(Exception(f'On-chain error: {onchain_exc}'))
    ...
    if DO_GALXE_TASKS:
        try:
            async with GalxeAccount(idx, account_info, evm_wallet) as galxe_account:
                ...
                logger.info(f'{idx}) Waiting for 30s before claim')
                await asyncio.sleep(30)
                for campaign_id in GALXE_CAMPAIGN_IDS:
                    await galxe_account.claim_campaign(campaign_id)

        except Exception as galxe_exc:
            excs.append(Exception(f'Galxe error: {galxe_exc}'))
...
async def process_batch(bid: int, batch, storage: AccountStorage, async_func, sleep):
    ...
    for idx, d in enumerate(batch):
        ...
        except MultiException as multi_exc:
            failed.append(d)
            for exc in multi_exc.excs:
                await log_long_exc(d[0], 'Process account error', exc)
...
def main():
    ...
    csv_data = [['#', 'EVM Address', 'Total Points',
                 'Drip $BERA', 'Swap $BERA', 'Mint $HONEY',
                 'Twitter Error']]
    campaigns = None
    total = {'total_points': 0, 'drip': 0, 'swap': 0, 'mint': 0, 'twitter_error': 0}
    for idx, w in enumerate(evm_wallets, start=1):
        ...
        total['drip'] += 1 if account.drip_bera else 0
        total['swap'] += 1 if account.swap_bera else 0
        total['mint'] += 1 if account.mint_honey else 0
        total['twitter_error'] += 1 if account.twitter_error else 0

        csv_data.append([idx, evm_address, total_points] + points +
                        [account.drip_bera_s, account.swap_bera_s, account.mint_honey_s, account.twitter_error_s])

    csv_data.append([])
    csv_data.append(['', '', total['total_points']] +
                    [f'{total.get(cn)[0]} / {total.get(cn)[1]}' if total.get(cn)[1] else total.get(cn)[0]
                     for _, cn in campaigns] +
                    [total['drip'], total['swap'], total['mint'], total['twitter_error']])
    csv_data.append(['', '', 'Total Points'] + [n for _, n in campaigns] +
                    ['Drip $BERA', 'Swap $BERA', 'Mint $HONEY', 'Twitter Error'])
